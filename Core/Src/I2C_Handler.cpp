...
HAL_Status I2C_Handler::Mem_Read(uint16_t devAddress, uint16_t memAddress, uint16_t memAddSize, uint8_t* data, uint16_t size) {
    if (!hi2c_) {
        return HAL_Status::HAL_ERROR;
    }
    return static_cast<HAL_Status>(HAL_I2C_Mem_Read(hi2c_, devAddress << 1, memAddress, 1, data, size, i2cTimeout));
}
